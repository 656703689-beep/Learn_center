...
- [ ] 四个函数签名完整（参数 + 返回值都有类型），`npx tsc --noEmit` 沉默
- [ ] 没有电话的联系人 add 成功，打印出来**整个没有 phone 字段**（不是 null）
- [ ] 能说出 remove / update 的"找不到"分支各返回什么、为什么这么选
- [ ] 实验 1：亲眼见过 TS2540，能解释为什么 readonly 拦不住"换新对象"；实验后已恢复沉默
- [ ] 实验 2：非法标签编译期被拦（TS2322 抄进注释），`type Tag` 改造保留在代码里
- [ ] 复查：没有给函数体内的局部变量逐个写注解（边界写死、内部放手）
...
*上一课：[第 1 课 · 跑起来 + 类型擦除](0001-type-erasure.md) ｜ 下一课：第 3 课 · 可空与窄化（完成本课后解锁）*
