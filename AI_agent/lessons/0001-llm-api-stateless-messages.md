...
> 今天不用装任何东西——Python 和 `requests` 你的机器上已经有了，我们连 SDK 都不用，直接裸调 HTTP。
...
3. **历史是你手里的普通数据**——想删掉一段、想改写、想总结压缩，都是改你自己的列表。后面所有 agent 的"记忆管理"，起点全在这里。

> ⚠️ **本课避坑**
> 别把"无状态"当成模型的缺陷。它正是 agent 可控性的来源：模型每次看到什么，完全由你决定——这意味着对话的上下文**可审计、可裁剪、可重放**。框架替你管历史，管的就是这个列表，仅此而已。
...
→ **[对照答案](0001-answers.md#sec-1-1)**（答案和提示单独放一个文件，防手滑瞟到）
  ...
  "choices": [                            // ← 为什么是数组？n>1 时一次出多个候选
    {
      ...
      "message": {
        "role": "assistant",              // ← 这整个对象，append 回你的 messages
...
- **裁剪历史是必修课**——删哪条、留哪条、怎么把老对话总结成一条，是"记忆管理"的全部内容。今天只要知道：这事发生在这个列表上。

---

## §3 练习：80 分钟，主菜上桌

全部代码放在 `01_API基础/`。先给你**起步骨架**——只给"调用的姿势"，多轮对话的维护是你今天的练习本体，别让我替你写了：
...
1. 新建 `ex1_minimal_call.py`，用骨架跑通，打印完整响应 JSON。
...
新建 `ex2_chat.py`。目标：一个支持多轮记忆的命令行聊天机器人——**记忆由你的代码维护**。

1. 程序开头初始化 `messages`（含一条你设计的 `system`）。
2. 循环：读用户输入 → append 成 `user` 消息 → 调 API → 把返回的 message append 回列表 → 打印回复。
3. 输入 `/exit` 退出。

- 💡 卡住了？[提示 1：请求和响应怎么接起来](0001-answers.md#hint-1) · [提示 2：细节清单](0001-answers.md#hint-2)（想清楚再点）

写完必须做**记忆测试**（这步比写代码本身重要）：

> ✅ **记忆测试**
> 第 1 轮告诉它："我叫小钟，我最喜欢的数字是 42"。中间聊 3–4 轮别的。然后问："我叫什么？最喜欢的数字是多少？"——它答得上来，说明历史真的被你传过去了；答不上来，先检查你的 append 逻辑，别怪模型。
...
- [ ] 任务 1–3 全部跑通，响应每个字段说得出作用
- [ ] 记忆测试通过：5 轮后模型仍记得第 1 轮的内容
- [ ] 能指着代码说出：模型的"记忆"来自哪一行
- [ ] 亲眼看过一次"失忆"：手动删掉 messages 中间两条再问，模型懵了（这一眼，比读十篇讲无状态的文章都管用）
- [ ] 能解释 prompt_tokens 为什么单调递增

> 💡 **卡住 20 分钟就求助**
> 卡住超过 20 分钟是信号，不是耻辱。把你**期望什么、实际发生什么、完整报错、相关代码**四样贴给我，别自己耗着——debugging 报错也是这 20 小时的一部分，我陪你 debug。

---

## §4 复盘：10 分钟检索练习

规则：**合上代码**，先在心里把答案完整说出来，再去[答案册](0001-answers.md)对照。"感觉我知道"不算数——说得出来才算。答完之后，把计划里的三道复盘问题口述成文字发给我，我来判卷：
...
→ [看答案](0001-answers.md#q1)

**Q2. 想让第 3 轮的模型彻底"忘记"第 2 轮，正确做法是？**
→ [看答案](0001-answers.md#q2)

**Q3. 想让模型全程用固定人设和规则回答，设定应放在哪里？**
→ [看答案](0001-answers.md#q3)

**Q4. 多轮对话进行到第 10 轮时，单次请求的 prompt token 大致怎么变？**
→ [看答案](0001-answers.md#q4)

**Q5. 第 2 轮请求里那条 assistant 消息，是谁写进去的？**
→ [看答案](0001-answers.md#q5)

---

## §5 下课

**学有余力（可选）**：给机器人加 `/reset` 清空历史、退出时把对话存成 JSON 文件——这是长期路线图等级 1 的正式练习，现在做等于提前打卡。
